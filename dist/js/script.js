...
$(document).ready(function (){
    ...
    let fullHeaderHeight = $(".header").height();
    ...
    $(window).on("scroll", function (){
        displayMiniHeader($(this).scrollTop(), fullHeaderHeight);
    });


    const btnOpen = $(".burger_open");
    const btnClose = $(".burger_close");
    ...
    // Sliders Start
    let swiperPromotions = new Swiper(".swiper-promotions", {
        pagination: {
            el: ".swiper-pagination",
            clickable: true,
        },
        ...
        grabCursor: true,
        navigation: {
            nextEl: '.swiper-promotions__btn_next',
            prevEl: '.swiper-promotions__btn_prev',
        },
    });
    ...
    let swiperCartDrop = new Swiper("#swiperCartDrop", {
        grabCursor: true,
        spaceBetween: 10,
        freeMode: true,
        slidesPerView: 'auto',
    ...
    let swiperCart = new Swiper("#swiperCart", {
        grabCursor: true,
        spaceBetween: 10,
        freeMode: true,
        slidesPerView: 4,
        ...
        speed: 600,
        navigation: {
            nextEl: '.swiper-cart__btn_next',
            prevEl: '.swiper-cart__btn_prev',
        },
        breakpoints: {
            300: {
                slidesPerView: 1,
    ...
    let swiperAdditCart = new Swiper("#swiperCartAddit", {
        grabCursor: true,
        spaceBetween: 20,
        freeMode: true,
        slidesPerView: 6,
    ...
    // Modal Start
    Fancybox.bind("[data-fancybox='productCard']", {
        dragToClose: false,
        hideScrollbar: true,
        groupAttr: false,
    });
    Fancybox.bind("[data-fancybox='loginModal']", {
        dragToClose: false,
        hideScrollbar: true,
        groupAttr: false,
    });
    Fancybox.bind("[data-fancybox='deliveryForm']", {
        dragToClose: false,
        hideScrollbar: true,
        groupAttr: false,
    });
    ...
    $("._sms").inputmask("9 9 9 9");
    $("._text").inputmask({ regex: "[A-Za-zА-Яа-я\\s]*" });
    $(".form-card__item_card").inputmask("9999 9999 9999 9999");
    $(".form-card__item_date").inputmask("99 / 99");
    $(".form-card__item_cvc").inputmask("999");
    $("#payChange").inputmask({ regex: "[0-9]*" });
    // Input Masks End

    $("#formLogin").on("submit", function (e){
        e.preventDefault();
        if ($("#loginPhone").inputmask("isComplete")){
            $(".login__row_sms").slideDown();
    ...
    $(".count-field__btn").on("click", function (){
        let operation = $(this).data("count-field-btn");
        let val = $(this).siblings(".count-field__num");
        changeCountFieldVal(operation, val);
    })

    $(".cart-head__btn").on("mouseover", function (){
        if (!isMobile.any() ) {
            $("#headerCart").slideDown();
        }
    });
    $("#headerCart").on("mouseleave", function (){
        if (!isMobile.any() ) {
            $("#headerCart").slideUp();
        }
    });
    ...
    $(document).mouseup(function (e){ // событие клика по веб-документу
        let div = $("#headerCart"); // тут указываем ID элемента
    ...
    $(".swiper-cart__link, .swiper-cart-addit__link").on("click", function (){
        $(this).toggleClass("_active");
    });

    $(".delivery-time__item").on("click", function (){
        $(".delivery-time__item").removeClass("_active");
        $(this).addClass("_active");
        $("#deliveryTimeField").val($(this).children(".delivery-time__text").text());
        Fancybox.close();
    });


    $(".execution-pay__check input:radio").on("change", function(){
        let currentParent = $(this).parents(".execution-pay__item");
        if ($(this).is(":checked")) {
            currentParent.siblings(".execution-pay__item").children(".execution-pay__content").slideUp("fast");
            currentParent.children(".execution-pay__content").slideDown("fast");
        }
    });
    $("#payWithoutChange:checkbox").on("change",function (){
        if ($(this).is(":checked")) {
            $("#payChange").attr("disabled", true).val("1 048");
        } else  {
            $("#payChange").attr("disabled", false).val("0");
        }
    });

    $(".form-input__hint").on("click", function (){
        let val = $(this).text();
        $(this).parents(".form-input__label").find(".form-input__input").val(val);
    });

    $(".delivery-address__type").on("click", function (){
        $(".delivery-address__type").removeClass("_active");
        $(this).addClass("_active");
        let dataType = $(this).data("type-delivery");
        $(".delivery-address__item").not(".delivery-address__item[data-type-delivery='" + dataType + "']").slideUp();
        $(".delivery-address__item[data-type-delivery='" + dataType + "']").slideDown();
    });

    $("#setDeliveryAddress").on("submit", function (event){
        event.preventDefault();
        let $inputs = $("#setDeliveryAddress :input");

        let values = {};
        $inputs.each(function() {
            values[this.name] = $(this).val();
        });
        setDeliveryField(values, 1);
        Fancybox.close();
    });
    $("#setPickupAddress").on("submit", function (event){
        event.preventDefault();
        let input = $("#setPickupAddress input:checked");

        setDeliveryField(input);
        Fancybox.close();
    });
});

function displayMiniHeader(positionY, headerHight) {
    const header = $(".header_full");
    if (positionY >= 75) {
        header.addClass("header_fixed");
        setHeaderPadding(headerHight, header.height());
    } else {
...
function displayMobileMenu(status = 0) {
...
function setHeaderPadding(oldHeight, newHeight) {
    let val = oldHeight - newHeight;
    $("body").css("padding-top", val);
}

function setProductTypeGroup(){
    const productTypeGroup = $(".product-type__group");
    productTypeGroup.each(function (){
        let countEl = $(this).find("label").length;
        if ($(window).width() > 499) {
            setProductTypeGroupSize($(this), countEl);
        } else {
            setProductTypeGroupSize($(this));
        }
    });
}

function setProductTypeGroupSize(el, count = 1) {
    el.css({
        "grid-template-columns" : "repeat(" + count + ", 1fr)",
    });
}

function changeCountFieldVal (operation, val) {
    let valData = val.attr("data-count-field-num");
    if (operation === "-") {
        if (valData > 0) {
            valData--;
            val.attr("data-count-field-num", valData);
            val.text(valData);
        }
    }
    if (operation === "+") {
...
function setDeliveryField(values, delivery = 0) {
    let outputValues = "";
    if (delivery === 1) {
            ...
            if (value !== "") {
                switch(index) {
                    case 'delivery_street':
                        outputValues += "Улица: " + value + "\n";
                        break;
                    case 'delivery_home':
                        outputValues += "Дом: " + value + "\n";
                        break;
                    case 'delivery_apart':
                        outputValues += "Квартира: " + value + "\n";
                        break;
                    case 'delivery_corp':
                        outputValues += "Подъезд: " + value + "\n";
                        break;
                    case 'delivery_domofon':
                        outputValues += "Домофон: " + value + "\n";
                        break;
                    case 'delivery_floor':
                        outputValues += "Этаж: " + value + "\n";
                        break;
                }
            }
        });
        $("#deliveryFieldOutput").val(outputValues);
        $("#deliveryFieldOutput").parents("label").find(".form-input__name").text("Адрес доставки");
    } else {
        let strings = values.parents(".delivery-address__input").find("p");
        $.each(strings, function (index, value){
            outputValues += value.innerText + "\n";
        });
        $("#deliveryFieldOutput").val(outputValues);
        $("#deliveryFieldOutput").parents("label").find(".form-input__name").text("Откуда забрать");
    }
}
...
// Определение использования мобильного устройства
isMobile = {
    Android: function() {
